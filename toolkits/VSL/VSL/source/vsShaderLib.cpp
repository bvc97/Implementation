...
void
VSShaderLib::loadShader(VSShaderLib::ShaderType st, std::vector<std::string> fileNames) {
	...
	std::string source;
	char ** sourceC;
	sourceC = (char **)malloc(sizeof(char*) * fileNames.size());
	for (int i = 0; i < fileNames.size(); ++i) {
		if (i > 0)
			source = "#line 1 " + std::to_string(i) + "\n";
		source += textFileRead(fileNames[i]);
		sourceC[i] = (char *)malloc(sizeof(char) * source.size() + 2);
		memcpy(sourceC[i], source.c_str(), source.size());
		char *aux = sourceC[i];
		aux[source.size()] = '\n';
		aux[source.size() + 1] = '\0';
	}
	...
	if (sourceC != NULL) {
		...
		pShader[st] = glCreateShader(spGLShaderTypes[st]);
		glShaderSource(pShader[st], (GLsizei)fileNames.size(), sourceC, NULL);
		glAttachShader(pProgram, pShader[st]);
		glCompileShader(pShader[st]);
