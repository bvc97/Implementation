...
#if (__VSL_TEXTURE_LOADING__ && __VSL_MODEL_LOADING__)
...
VSModelLib::genVAOsAndUniformBuffer(const struct aiScene *sc) {
	...
	for (unsigned int n = 0; n < sc->mNumMeshes; ++n)
	{
		...
		aMesh.faceArray.resize(mesh->mNumFaces * 3);
		...
		for (unsigned int t = 0; t < mesh->mNumFaces; ++t) {
			...
			memcpy(&aMesh.faceArray[faceIndex], face->mIndices,
		...
		if (pUseAdjacency) {
			...
			for (unsigned int i = 0; i < mesh->mNumFaces; ++i) {
				...
				edge[i*3].vertex = aMesh.faceArray[i*3+1];
				edge[i*3+1].vertex = aMesh.faceArray[i*3+2];
				edge[i*3+2].vertex = aMesh.faceArray[i*3];
				...
				myEdges[std::pair<unsigned int,unsigned int>(aMesh.faceArray[i*3+2], aMesh.faceArray[i*3])] = &edge[i*3];
				myEdges[std::pair<unsigned int,unsigned int>(aMesh.faceArray[i*3], aMesh.faceArray[i*3+1])] = &edge[i*3+1];
				myEdges[std::pair<unsigned int,unsigned int>(aMesh.faceArray[i*3+1], aMesh.faceArray[i*3+2])] = &edge[i*3+2];
			}
		...
		if (pUseAdjacency) {
			...
			glBufferData(GL_ELEMENT_ARRAY_BUFFER,
						sizeof(unsigned int) * aMesh.numIndices,
						&aMesh.faceArray[0], GL_STATIC_DRAW);
		...
		if (mesh->HasPositions()) {
			...
			//std::vector<float> pp; 
			aMesh.positions.resize(4 * mesh->mNumVertices);
			for (unsigned int k = 0; k < mesh->mNumVertices; ++k) {
				aMesh.positions[k * 4] = mesh->mVertices[k].x;
				aMesh.positions[k * 4 + 1] = mesh->mVertices[k].y;
				aMesh.positions[k * 4 + 2] = mesh->mVertices[k].z;
				aMesh.positions[k * 4 + 3] = 1.0f;;
			}
			glBufferData(GL_ARRAY_BUFFER, sizeof(float)* aMesh.positions.size(), 	&aMesh.positions[0], GL_STATIC_DRAW);
	...
	if (nd->mNumMeshes)
	{
		...
		for (unsigned int n = 0; n < nd->mNumMeshes; ++n) {
			...
			if(sc->mMeshes[nd->mMeshes[n]]->mPrimitiveTypes == 4) {
				...
				//							sizeof (aMesh));
				memcpy(mMyMeshes[nd->mMeshes[n]].transform,mVSML->get(VSMathLib::AUX0),
											sizeof(float)*16);
#ifndef __ANDROID_API__
				if (pUseAdjacency)
