...
VSSolidGrid::VSSolidGrid() : mMax(1.0f), mDivisions(10) {}


void
VSSolidGrid::set(float max, int divisions) {
...
VSSolidGrid::prepare() {
	...
	for (int i = 0; i < mDivisions - 1; ++i) {
		for (int j = 0; j < mDivisions - 1; ++j) {
			startX = (i - mDivisions * 0.5f) * step;
			startY = (j - mDivisions * 0.5f) * step;

			ind.push_back(i * mDivisions + j);
			ind.push_back(i * mDivisions + j + 1);
			ind.push_back((i+1) * mDivisions + j);

			ind.push_back((i + 1) * mDivisions + j);
			ind.push_back(i * mDivisions + j + 1);
			ind.push_back((i + 1) * mDivisions + j+1);
		}
	}
	...
	for (int i = 0; i < mDivisions; ++i) {
		for (int j = 0; j < mDivisions; ++j) {
			...
			p.push_back(startX); p.push_back(0.0f); p.push_back(startY); p.push_back(1.0f);
			n.push_back(0.0f); n.push_back(1.0f); n.push_back(0.0f); 
			t.push_back(i * 1.0f / (mDivisions-1)); t.push_back(j * 1.0f / (mDivisions - 1));
		}
	}

	MyMesh aMesh;

	buildVAO(aMesh, p.size() / 4, &(p[0]), &(n[0]), &(t[0]), NULL, NULL, ind.size(), &(ind[0]));
