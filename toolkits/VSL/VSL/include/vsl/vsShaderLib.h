...
public:
	...
	*/
	...
	/** Loads the text in the file to the source of the specified shader 
	  ...
	  *	\param filename the file name where the source is to be found
	*/
	void loadShader(VSShaderLib::ShaderType st, std::string fileName);

	/** Loads the text in the files to the source of the specified shader
	  *
	  * \param st one of the enum values of ShaderType
	  *	\param filenames a vector with the file names where the source is to be found
	  */
	void loadShader(VSShaderLib::ShaderType st, std::vector<std::string> fileNames);
