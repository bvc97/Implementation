...
static vec3 pos_cam = scale_vec3(50, { 1.0,1.0,1.0 });
...
size_t max_vertices_per_boat = 1 << 10;
...
void drawScene() {
    ...
    // � preciso sincronizar os acessos a memoria com glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT) https://registry.khronos.org/OpenGL-Refpages/gl4/html/glMemoryBarrier.xhtml 

    // se o triangulo � cortado existem 9 vertices, senao, os ultimos 6 sao preenchidos a 0
    //               t      t      t
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, max_vertices_per_boat * NUM_BOATS * sizeof(vec4), NULL, GL_DYNAMIC_DRAW);
    ...
    glMemoryBarrier(GL_ALL_BARRIER_BITS);
    //glDispatchCompute(int(pow(2, 10)), 1, 1);
    //chamar computer shader
    int ea = min(1 << 10, (int)(max_boats * max_vertices_per_boat));
    int eb = (int)(max_boats * max_vertices_per_boat) / ea;
    glDispatchCompute(ea, eb, 1);
    /*glBindBuffer(GL_SHADER_STORAGE_BUFFER, forces);
    ...
    for (int i = (int(log2(max_boats)) + int(log2(max_vertices_per_boat)) - 1); i >= int(log2(max_boats)); i--) {
        int a = min(10, i);
        int b = i - a;
        glMemoryBarrier(GL_ALL_BARRIER_BITS);
        //chamar computer shader
...
GLuint setupShaders() {
    ...
    // Shader for test
    waves.init();
    waves.loadShader(VSShaderLib::VERTEX_SHADER, path + "shaders_/waves.vert");
    waves.loadShader(VSShaderLib::FRAGMENT_SHADER, path + "shaders_/waves.frag");
    ...
    // Shader for test
    ocean.init();
    ocean.loadShader(VSShaderLib::VERTEX_SHADER, path + "shaders_/ocean.vert");
    ocean.loadShader(VSShaderLib::FRAGMENT_SHADER, path + "shaders_/ocean.frag");
    ...
    // Shader for test
    algorithm.init();
    algorithm.loadShader(VSShaderLib::VERTEX_SHADER, path + "shaders_/cut.vert");
    algorithm.loadShader(VSShaderLib::GEOMETRY_SHADER, path + "shaders_/cut.geom");
    algorithm.loadShader(VSShaderLib::FRAGMENT_SHADER, path + "shaders_/cut.frag");
    ...
    // Shader for test
    forces_zero.init();
    forces_zero.loadShader(VSShaderLib::COMPUTE_SHADER, path + "shaders_/forces_zero.comp");
    ...
    // Shader for test
    forces_store.init();
    //forces_store.loadShader(VSShaderLib::COMPUTE_SHADER, path + "shaders_/forces_store.comp");
    forces_store.loadShader(VSShaderLib::VERTEX_SHADER, path + "shaders_/forces_store.vert");
    forces_store.loadShader(VSShaderLib::GEOMETRY_SHADER, path + "shaders_/forces_store.geom");
    forces_store.loadShader(VSShaderLib::FRAGMENT_SHADER, path + "shaders_/forces_store.frag");
    ...
    // Shader for test
    forces_sum.init();
    forces_sum.loadShader(VSShaderLib::COMPUTE_SHADER, path + "shaders_/forces_sum.comp");
    ...
    // Shader for test
    torques_sum.init();
    torques_sum.loadShader(VSShaderLib::COMPUTE_SHADER, path + "shaders_/torques_sum.comp");
    ...
    forces_application.loadShader(VSShaderLib::VERTEX_SHADER, path + "shaders_/forces_application.vert");
    forces_application.loadShader(VSShaderLib::GEOMETRY_SHADER, path + "shaders_/forces_application.geom");
    forces_application.loadShader(VSShaderLib::FRAGMENT_SHADER, path + "shaders_/forces_application.frag");
    ...
    boat_length_program.loadShader(VSShaderLib::COMPUTE_SHADER, path + "shaders_/boat_length.comp");
