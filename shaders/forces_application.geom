...
vec4 next_a_velocity(vec3 w, vec4 T, float d_t) {
	...
	vec4 result = vec4(w,0) + d_t * a_angular;
    result.w = 0;
    return result;
}
...
void apply_translate(vec3 A0, vec3 B0, vec3 C0, vec3 A1, vec3 B1, vec3 C1, vec3 A2, vec3 B2, vec3 C2){
    vec3 gravity = boat_mass[boat_idx] * vec3(0.0, -9.8, 0.0);
    
    vec3 F = force[0].xyz + gravity;
...
void apply_rotate(vec3 A0, vec3 B0, vec3 C0, vec3 A1, vec3 B1, vec3 C1, vec3 A2, vec3 B2, vec3 C2){
    vec4 new_w = next_a_velocity(vel_angular[boat_idx].xyz, torque[0], delta_t);
...
void main()
{
    ...
    apply_translate(A0, B0, C0, A1, B1, C1, A2, B2, C2);

    apply_rotate(A0, B0, C0, A1, B1, C1, A2, B2, C2);
