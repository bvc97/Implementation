...
float submerged_triangle_area(vec3 A, vec3 B, vec3 C) {
	...
	float submerged_triangle_area = area(A, B, C) * cos_theta(A, B, C);

	return submerged_triangle_area;
}

vec3 slamming_force(vec3 A, vec3 B, vec3 C){
	return boat_ATotal[boat_idx] == 0 ? vec3(0.0) : ((2 * boat_mass[boat_idx] * submerged_triangle_area(A, B, C)) / boat_ATotal[boat_idx]) * vel[boat_idx].xyz;
}
